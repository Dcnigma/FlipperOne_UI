...
A web application that simulates the Flipper One LCD screen user interface. It renders a 1024x600 monochrome (black/white/grayscale) display using HTML5 Canvas and runs inside the Cog web browser on the actual Flipper One hardware.
...
- **HTML5 Canvas** — pixel-perfect 1024x600 rendering
...
The goal is to run Cog directly on the 1024x600 SPI LCD without KDE. Cog's DRM plugin (`-P drm`) only works with GPU-backed connectors (HDMI on card2), not the SPI panel on card0. The solution is **cage** — a minimal Wayland kiosk compositor that handles non-GPU displays and runs one app fullscreen.
...
DRM devices:
- `/dev/dri/card0` — SPI LCD panel (1024x600, panel-mipi-dbi-spi)
...
- **Detail view**: 3× zoomed 1024×600 canvas with a light-gray/white checker background (so transparent pixels are visible), a 1-native-pixel grid overlay (toggleable), and auto-generated controls that match the selected component's `tweakables` schema. `← back to library` returns to the list.
...
1. Add a static `tweakables` schema on the component (outside the IIFE):
   ...
   MyComponent.tweakables = [
       { section: 'position' },
       { key: 'x', type: 'range', min: 0, max: 1024, default: 0 },
       // ...
   ];
   ```
...
Test pattern page for verifying LCD display alignment — draws 1px border, diagonal cross, center crosshair, and corner coordinate labels at native 1024x600.
...
└── js/
    ...
    └── apps/
        ...
        ├── figma_live_preview.js  # Embeds a Figma prototype in a 1024×600 iframe; shows a mix-blend "x2 to exit" hint
...
- **Foreground color**: White (`#ffa550`). Grayscale sprites preserve their
...
**Coloring**:
- **White (`#ffa550`)** — filled bars (`ceil(quality / 100 * 5)`)
...
- **Size**: 7×7 sprite, rendered via `canvas.drawSprite(..., '#ffa550')`
...
- **Rendered** with `canvas.drawSprite(..., '#ffa550')`
...
- **Font**: HaxrcorpFont16, color `#ffa550`
...
### Button Bar Layout
- **5 button slots** (48px each, 2px gap): `5 × 48 + 4 × 2 = 1024px` exactly
...
### Drawing Sprites
```javascript
var spriteX = Math.floor((1024 - Dolphins.sprite.w) / 2);  // center
...
- **Dimensions**: 1024px × 600px (width × height)
...
128–600px:    Button bar (5 button positions)
```

### Coordinate System
- **Origin**: Top-left (0, 0)
- **Left/Right margins**: 8px from canvas edges
- **Centered content width**: 240px (canvas 1024px - 16px margins)
...
| `BTN_W` (bottom bar) | 48px | `menu.js` | Width of a single button slot (5 × 48 + 4 × 2 = 1024) |
...
| `pressed` | Icon + text flip to `#ffa550`. Animation keeps advancing (clock preserved across `SELECTED ↔ PRESSED`). | Same `MenuSelectorFrame` instance but with `showFill: true`, `fillColor: '#000'` — drawn *behind* the line content, then the line is redrawn on top so the white content reads over the black fill. | `Born2bSportyV2Medium` |
...
| pressed | `drawRoundRect(..., 2, '#000')` — filled black, text in `#ffa550` (selection frame suppressed while pressed) |
...
y: 130 ── 600     Bottom button bar (BTN_H = 14, 5 × 48px slots + 4 × 2px gaps)
```
...
var x = Math.floor((1024 - textWidth) / 2);
...
- `CONTAINER_X = 16`, `CONTAINER_Y = 24` (below the 13px status bar with
  ...
  intentional)
- `CONTAINER_W = 224` (`1024 − 16 − 16`)
- No background fill of its own; the canvas clear is `#ffa550`

**Items**: `MenuLine` instances stacked flush inside the container with
a 1px gray (`#824704`) horizontal divider between each adjacent pair.
...
2. `render` paints the selector frame filled black behind the line,
   then redraws the (now inverted) line on top so its `#ffa550` content
...
- **Width**: 48px each, 2px gap between (5 × 48 + 4 × 2 = 1024px exact)
- **Height**: **14px** (`BTN_H` in `canvas.js`) — sits flush at the bottom of the 600px canvas (`y = 130`)
...
MyScene.prototype.render = function(canvas) {
    ...
    if (this.popup) {
        ...
        canvas.ctx.fillRect(0, 0, 1024, 600);
...
canvas.drawSprite(Icons.myIcon, x, y, '#ffa550');  // White (for pressed state)
```
...
canvas.drawSprite(Icons.network, 10, 50, '#ffa550');
...
- Works with any HTML color (#000, #ffa550, #f00, etc.)
...
MenuLine.prototype.render = function(canvas, x, y) {
    ...
    if (this.state === STATE_PRESSED) {
        textColor = '#ffa550';
        iconColor = '#ffa550';   // line draws icon/text in white over the
...
| −1   | tab below | 13 px tab, label `#faa134666` |
| −2   | deeper tab below | 13 px tab, lighter `#f5b153`, anchored to the canvas bottom |
