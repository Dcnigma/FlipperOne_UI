var FlipCanvas = (function() {
    ...
    // The actual <canvas> element is PHYS_W x PHYS_H (1024x600).
    // Every existing drawMiddleButton / drawKeyboard / etc. method
    // in this file was authored in pixel-art terms against a
    // 256x144 logical canvas (this.w / this.h below) — rounded
    // corners, bitmap font glyphs, and icon bitmaps are all
    // hand-placed 1px fillRects tuned for that grid.
    //
    // 1024/256 = 4 exactly, but 600/144 = 4.1666... (NOT a whole
    // number — 144*4 = 576, not 600). A non-integer scale factor
    // on pixel art causes uneven row heights and wobbly borders,
    // so instead of stretching to fill 600px we scale uniformly
    // by LOGICAL_SCALE (4) into a 1024x576 content area, then
    // centre that vertically inside the real 600px canvas. The
    // remaining 24px (12px top + 12px bottom) are letterboxed —
    // cleared to a margin colour rather than stretched.
    //
    // Every prototype method below is UNCHANGED: it still draws
    // in 0..256 / 0..144 logical coordinates. The scale + vertical
    // offset is applied once, as a ctx transform, in the
    // constructor and re-applied at the top of `clear()` (the one
    // place every render frame is guaranteed to pass through).
    var PHYS_W = 1024;
    var PHYS_H = 600;
    var LOGICAL_W = 256;
    var LOGICAL_H = 144;
    ...
    function FlipCanvas(canvasEl) {
        this.el = canvasEl;
        // Logical dimensions — every existing drawing method in
        // this file reads `this.w` / `this.h` assuming this space.
        // Do NOT set these to the physical 1024x600; that would
        // make every `this.h - BTN_H` bottom-anchor (and similar)
        // compute against the wrong canvas height.
        this.w = LOGICAL_W;
        this.h = LOGICAL_H;
        // Physical element size — what's actually visible/allocated.
        canvasEl.width  = PHYS_W;
        canvasEl.height = PHYS_H;
        this.physW = PHYS_W;
        this.physH = PHYS_H;
        this.scale = LOGICAL_SCALE;
        this.letterboxY = LETTERBOX_Y;
        this.ctx = canvasEl.getContext('2d');
        this.ctx.imageSmoothingEnabled = false;
        this._applyTransform();
    }

    // Reset to the identity transform, then scale + translate so
    // (0,0) in logical space lands at the top of the centred
    // content band, and 1 logical unit = LOGICAL_SCALE physical
    // pixels on both axes. Safe to call repeatedly (e.g. every
    // `clear()`) since `setTransform` always starts from identity
    // rather than compounding onto whatever transform was active.
    FlipCanvas.prototype._applyTransform = function() {
    ...
    FlipCanvas.prototype.clear = function(color) {
        // Letterbox bars first, in PHYSICAL (untransformed) space —
        // reset to identity temporarily so the fill covers the
        // real 1024x600 canvas regardless of the logical transform.
        var ctx = this.ctx;
        ctx.save();
        ctx.setTransform(1, 0, 0, 1, 0, 0);
        ctx.fillStyle = '#000';
        ctx.fillRect(0, 0, this.physW, this.physH);
        ctx.restore();
        // Re-apply the logical transform (defensive — in case
        // something upstream changed it) and fill the visible
        // 256x144 logical content area in the requested colour.
        this._applyTransform();
    ...
    FlipCanvas.prototype.drawRect = function(x, y, w, h, color) {
        this.ctx.fillStyle = color || '#ffa550';
    ...
    function _btnColors(pressed) {
        return { bg: pressed ? '#000' : '#ffa550', fg: pressed ? '#ffa550' : '#000' };
    }
    ...
    // on, light gray (#824704) when off, so the button reads as a
    ...
    FlipCanvas.prototype.drawText = function(text, x, y, color, scale) {
        BitmapFont.draw(this.ctx, text, x, y, color || '#ffa550', scale);
    };
    ...
    // Body: white (#ffa550), Tab: darker (#4d4d4d)
    ...
    FlipCanvas.prototype.drawKeyboard = function(x, y, rows, selectedRow, selectedCol, pressedRow, pressedCol, bg, fg, shiftState, langLabel, peekWave, focused, betweenCellsAndSelector) {
        ...
        var SPACE_W = 15;  // Width of space button
        var SHIFT_W = 15;  // Width of shift button (matches space)
        var LANG_W  = 15;  // Width of language-switcher button (matches shift)
        ...
        for (var row = 0; row < rows.length; row++) {
            ...
            for (var col = 0; col < rowKeys.length; col++) {
                ...
                if (isPeekRow) {
                    ...
                    var peekTextColor = peekFilled ? '#ffa550' : '#000';
                    this.drawRoundRect(btnX, btnY, btnW, BTN_H, 2, peekBg);
                    if (!peekFilled) {
                        this.drawRoundFrame(btnX, btnY, btnW, BTN_H, 2, '#824704');
                    }
                ...
                if (isSelected) {
                    ...
                    if (isFilled) {
                        ...
                        paintContent(cell, btnX, btnY, btnW, '#ffa550');
                    } else {
